...
const template = createTemplate(
  `<div class=prism-search-container style=display:flex;align-items:flex-start;justify-content:flex-end><div dir=ltr class=prism-search><button type=button aria-expanded=false title="切换替换" class=pce-expand></button><div spellcheck=false><div><div class="pce-input pce-find"><input autocorrect=off autocapitalize=off placeholder=查找 aria-label=查找><button type=button class=prev-match title="上一个匹配 (Shift+Enter)"></button><button type=button class=next-match title="下一个匹配 (Enter)"></button><div class=search-error></div></div><button type=button class=pce-close title="关闭 (Esc)"></button></div><div class="pce-input pce-replace"><input autocorrect=off autocapitalize=off placeholder=替换 aria-label=替换><button type=button title=(Enter)>替换</button><button type=button title=(${isMac ? "Cmd" : "Ctrl+Alt"}+Enter)>全部</button></div><div class=pce-options><div class=pce-match-count>0<span> / </span>0</div><button type=button aria-pressed=false class=pce-regex title="正则表达式搜索${shortcut}R)"><span aria-hidden=true></span></button><button type=button aria-pressed=false title="保持大小写${shortcut}P)"><span aria-hidden=true>Aa</span></button><button type=button aria-pressed=false class=pce-whole title="匹配整个单词${shortcut}W)"><span aria-hidden=true>ab</span></button><button type=button aria-pressed=false class=pce-in-selection title="在选择范围内查找${shortcut}L)">`
);
